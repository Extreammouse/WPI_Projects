...
class Tree(object):
    ...
    def conditional_entropy(Y, X):
        ...
        total = len(X)
        ce = 0
        for values, count in count.items():
            index = np.where(X == values)[0]
            y_child = Y[index]
            ce += (count / total ) * Tree.entropy(y_child)
        #########################################
        return ce
    ...
    def split(X, Y, i):
        ...
        C = {}
        for j in label:
            attr_ind = np.where(matrix == j)[0]
            X_subset = X[:, attr_ind]
            Y_subset = Y[attr_ind]
            C[j] = Node(X_subset, Y_subset)
        #########################################
        return C
    ...
    def inference(t, x):
        ...
        if t.isleaf:
            ...
            if attribute_value in t.C:
                y = Tree.inference(t.C[attribute_value], x)
            else:
                y = t.p
